...
import { BookOpen, Users, Building2, Search, ArrowUpRight, ArrowDownLeft, X, Plus } from 'lucide-react';
...
export default function Ledger() {
  ...
  const [supplierPayments, setSupplierPayments] = useState(() => {
    const saved = localStorage.getItem('sbbj_supplier_payments');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    ...
    setSalesBills(JSON.parse(localStorage.getItem('sbbj_sales_bills')) || []);
    setPurchaseBills(JSON.parse(localStorage.getItem('sbbj_purchases_v2')) || []);
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem('sbbj_supplier_payments', JSON.stringify(supplierPayments));
  }, [supplierPayments]);
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                ...
                <tbody>
                  ...
                  {customerLedgers.length === 0 && (
                    <tr><td colSpan="2" style={{ padding: '2rem', textAlign: 'center', color: 'var(--clr-text-dim)' }}>No customers found.</td></tr>
                  )}
                </tbody>
              </table>
            ) : (
